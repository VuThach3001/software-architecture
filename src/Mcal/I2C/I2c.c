...
#define I2C_XFER_TIMEOUT_MS (-1) /* -1 means wait forever */
    ...
    /** @brief I2C device handles per job (one device handle per slave address) */
    static i2c_master_dev_handle_t s_i2c_dev_handles[I2C_MAX_JOBS] = {NULL};
#endif

    /** @brief Stored configuration from I2C_Init */
    static const I2C_ConfigType *s_i2c_config = NULL_PTR;

    /** @brief External Buffer configurations per Job */
/**
 * @file       I2c.c
 * @copyright  Copyright (C) 2026 ESP32-TYH5HC. All rights reserved.
 * @license    This project is released under the ESP32-TYH5HC License.
...
#define I2C_XFER_TIMEOUT_MS  (-1)
     ...
     */
    void (*I2C_SeqEndNotification)(I2C_SequenceType       SequenceId,
                                   I2C_SequenceResultType Result) = NULL_PTR;
    ...
    static void I2C_Prv_NotifySeqEnd(I2C_SequenceType       SequenceId,
    ...
    void I2C_Init(const I2C_ConfigType *ConfigPtr)
    {
        ...
        else
        {
            ...
            i2c_slave_config_t slave_cfg = {
                ...
                .scl_io_num                   = ConfigPtr->scl_pin,
                .clk_source                   = I2C_CLK_SRC_DEFAULT,
                .slave_addr                   = (uint16_t)ConfigPtr->deviceAddress,
            ...
            if (ConfigPtr->targetListening == TRUE)
            {
                ...
                ESP_ERROR_CHECK(i2c_slave_register_event_callbacks(
                    s_i2c_slave_handle, &cbs, (void *)(uintptr_t)0u));
                               ...
                               I2C_NumberOfDataType  Length)
    {
        ...
        I2C_AddressType resolvedAddr = (NodeAddress != 0u)
                                           ? NodeAddress
                                           : ((s_i2c_config != NULL_PTR)
                                                  ? s_i2c_config->deviceAddress
                                                  : 0u);
        ...
        if ((s_i2c_config != NULL_PTR) &&
            (s_i2c_config->hwUnitMode == I2C_HW_UNIT_MODE_CONTROLLER))
        {
    ...
    Std_ReturnType I2C_AsyncTransmit(I2C_SequenceType SequenceId)
    {
        ...
        esp_err_t err = i2c_master_register_event_callbacks(
            s_i2c_dev_handles[jobId], &cbs, (void *)(uintptr_t)SequenceId);
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "AsyncTransmit: callback register failed Seq %u (0x%x)",
                     SequenceId, err);
            return E_NOT_OK;
        }
        ...
        if (ret != E_OK)
        {
            ...
            (void)i2c_master_register_event_callbacks(s_i2c_dev_handles[jobId],
                                                      &cbs_null, NULL);
            return E_NOT_OK;
        }
#elif CPUVENDOR_SC == CPUVENDOR_SC_STM32
    ...
    Std_ReturnType I2C_StartListening(I2C_SequenceType SequenceId)
    {
        ...
        esp_err_t err = i2c_slave_register_event_callbacks(
            s_i2c_slave_handle, &cbs, (void *)(uintptr_t)SequenceId);
        if (err != ESP_OK)
        {
            ...
            if (s_i2c_config->asyncMode == I2C_POLLING_MODE)
            {
                ...
                if (s_slave_rx_done == TRUE)
                {
                    ...
                    else
                    {
                        ...
                        if ((jobId < I2C_MAX_JOBS) &&
                            (s_i2c_job_eb[jobId].txDataBufferPtr != NULL_PTR))
                        {
                            uint32_t wlen = 0u;
    ...
    static Std_ReturnType I2C_Prv_AddDevice(I2C_JobType JobId, I2C_AddressType NodeAddress)
    {
        ...
        i2c_device_config_t dev_cfg = {
            ...
            .scl_speed_hz    = (s_i2c_config != NULL_PTR)
                                   ? s_i2c_config->baudRate
                                   : 100000u,
        };

        esp_err_t err = i2c_master_bus_add_device(s_i2c_bus_handle, &dev_cfg,
                                                  &s_i2c_dev_handles[JobId]);
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "AddDevice: 0x%04X Job %u failed (0x%x)",
                     NodeAddress, JobId, err);
            return E_NOT_OK;
        }
    ...
    static Std_ReturnType I2C_Prv_ExecuteJob(I2C_JobType JobId)
    {
        ...
        {
            err = i2c_master_transmit_receive(devHdl,
                                             (const uint8_t *)*job->txDataBufferPtr,
                                             (size_t)job->length,
                                             (uint8_t *)*job->rxDataBufferPtr,
                                             (size_t)job->length,
                                             I2C_XFER_TIMEOUT_MS);
        }
        else if (hasTx == TRUE)
        {
            err = i2c_master_transmit(devHdl,
                                     (const uint8_t *)*job->txDataBufferPtr,
                                     (size_t)job->length,
                                     I2C_XFER_TIMEOUT_MS);
        }
                                             ...
                                             void                          *arg)
    {
        ...
        if ((s_i2c_config != NULL_PTR) &&
            (s_i2c_config->asyncMode == I2C_POLLING_MODE))
        {
                                          ...
                                          void                                 *arg)
    {
        ...
        if ((jobId < I2C_MAX_JOBS)                             &&
            (s_i2c_job_eb[jobId].rxDataBufferPtr != NULL_PTR)  &&
            (evt_data != NULL_PTR)                             &&
            (evt_data->buffer != NULL_PTR))
        {
            uint32_t copyLen = ((uint32_t)evt_data->length <
                                (uint32_t)s_i2c_job_eb[jobId].length)
        ...
        else
        {
            ...
            if ((s_i2c_config != NULL_PTR) &&
                (s_i2c_config->targetListening == FALSE))
            {
                s_listening_active = FALSE;
                s_i2c_status       = I2C_IDLE;
            }
